...
| D017 | 2026-08-26 | macOS ships as a native-architecture PyInstaller `.app` using PyWebView Cocoa/WebKit; keep the tested Python save engine instead of rewriting binary logic in Swift | Produces a self-contained Mach-O app now while preserving save-integrity coverage; ADR 0003 compares the alternatives |
...
- 2026-08-26: Native macOS arm64 bundle added — Cocoa/WebKit backend, platform-marked dependencies, PyInstaller `.app`, one-click build script, CrossOver/Whisky/Steam discovery, 173 tests on clean macOS host, signed bundle + live heartbeat/API smoke test.
- 2026-08-26: Public README fully rewritten in English for the macOS edition — Apple Silicon `arm64` status, compatibility matrix, build/run steps, save discovery, architecture, Gatekeeper guidance, limitations, safety and credits.
- 2026-08-31: macOS build regenerated with the existing Build_macOS.command on this host (Homebrew Python 3.14.7, PyInstaller 6.22.2). 173 tests discovered, 22 skipped, no failures; invariants and context lint pass. arm64 bundle is 60 MiB, ad-hoc signature verified; packaged API, native UI heartbeat and Origin guard smoke checks pass. No application source changes.
- 2026-08-31: macOS icon fixed: change_of_heart.icns is a direct conversion of the existing 1024px JPG logo, with standard and Retina sizes. P5R_Save_Editor.spec explicitly supplies this icon to BUNDLE. Rebuilt app embeds the identical icon, CFBundleIconFile points to it, and the ad-hoc signature verifies. Keep the ICNS alongside the spec for future builds.
