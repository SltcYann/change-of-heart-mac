...
(default: ~/p5r_buff_save, override with P5R_ORACLE_DIR). All mutated outputs are
written only to the
...
# Oracle save corpus lives under the user's home dir by default.
# Override with the P5R_ORACLE_DIR environment variable if kept elsewhere.
ORACLE_DIR = os.environ.get("P5R_ORACLE_DIR") or os.path.join(
    os.path.expanduser("~"), "p5r_buff_save"
)
...
def mutation_cycle(raw: bytes, slot: str, *, write_scratch: bool = False,
    ...
    party_slot = MUTATION["party_slot"]
    if ed.is_member_joined(party_slot):
        # normal path: joined member -> edit must succeed and survive reload
        st["party"] = ed.set_party_stat(party_slot, hp=MUTATION["party_hp"],
                                        sp=MUTATION["party_sp"]).get("status")
        assert st["party"] == "success", f"party edit failed: {st['party']!r}"
    else:
        # un-joined slot (seed state, e.g. DATA16) -> guard must refuse AND
        # leave the payload byte-identical (anti-corruption check)
        before = ed.parser.data_payload
        st["party"] = ed.set_party_stat(party_slot, hp=MUTATION["party_hp"],
                                        sp=MUTATION["party_sp"]).get("status")
        assert st["party"] == "invalid", f"un-joined slot edit not refused: {st['party']!r}"
        assert ed.parser.data_payload == before, "refused party edit changed payload!"
    ...
    if st["party"] == "success":
        ...
        # un-joined slot: guard refused the edit, so seeds must remain
        checks["party"] = (party["hp"] != MUTATION["party_hp"]
                           and party["sp"] != MUTATION["party_sp"])
        assert checks["party"], f"refused party edit still wrote values: {party!r}"
